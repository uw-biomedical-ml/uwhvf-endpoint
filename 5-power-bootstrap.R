    ...
    for (i in seq(500)) {
        ...
        if (setting == "model") {
            ...
            for (yr in times) {
                ...
                boots <- moddat[sample(nrow(moddat), floor(0.5 * nrow(moddat)), replace=T),]
                mod <- survfit(Surv(time, status) ~ 1, data=boots)
                out <- summary(mod, times = yr)
                eventrates$surv[eventrates$time == yr] <- out$surv[1]
            }
        } else {
            boots <- sdat[sample(nrow(sdat), floor(0.5 * nrow(sdat)), replace=T),]
            mod <- survfit(Surv(time, event) ~ 1, data=boots)
        ...
        for (i in seq(length(eventrates$time))) {
            ...
            for (effectsize in seq(0.10, 0.50, 0.10)) {
                ...
                tryCatch( {
                    ...
                    allres <- rbind(allres, data.frame(setting=setting, boot=i, length=yr, control.event = prob, drug.event = prob2, sample.size = pwr$n, effect.size=effectsize))
                }, error=function(e) e)
...
write.csv(allres, "data/boostrap.csv", row.names=F)
summ <- summarySE(allres, measurevar = "sample.size", groupvars=c("length", "effect.size", "setting"))
...
p <- ggplot(effect20, aes(Years, sample.size, color=setting, group=setting)) + geom_point(position=pd) + 
        geom_line(position=pd) + 
        geom_errorbar(aes(ymin=sample.size - ci, ymax=sample.size + ci), width=0.4, position=pd) + 
...
ggsave("figures/effect20-boot.pdf", p, width=12, height=6)
